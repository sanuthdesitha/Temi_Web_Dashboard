...
@login_required
def api_test_mqtt():
    """Test MQTT cloud broker connection"""
    global cloud_monitor
    ...
    try:
        if not cloud_monitor:
            return jsonify({
                'success': False,
                'error': 'MQTT cloud monitor not initialized'
            }), 400

        # Check if already connected
        if cloud_monitor.connected:
            return jsonify({
                'success': True,
                'message': 'Cloud MQTT broker connected successfully',
                'status': 'connected',
                'broker': cloud_monitor.broker_url,
                'port': cloud_monitor.port
            })
        ...
        if cloud_monitor.connect():
            return jsonify({
                'success': True,
                'message': 'Cloud MQTT broker connection successful',
                'status': 'connected',
                'broker': cloud_monitor.broker_url,
                'port': cloud_monitor.port
            })
            ...
            return jsonify({
                'success': False,
                'error': 'Failed to connect to cloud MQTT broker',
                'broker': cloud_monitor.broker_url,
                'port': cloud_monitor.port
            }), 400
        ...
        return jsonify({
            'success': False,
            'error': f'MQTT test failed: {str(e)}'
        }), 400
...
@login_required
def api_mqtt_status():
    """Get MQTT connection status for cloud broker and all robots"""
    global cloud_monitor
    ...
    # Cloud MQTT status
    if cloud_monitor:
        status_data['cloud_broker'] = {
            'connected': cloud_monitor.connected,
            'broker': cloud_monitor.broker_url,
            'port': cloud_monitor.port,
            'username': cloud_monitor.username,
            'message': 'Connected' if cloud_monitor.connected else 'Disconnected'
        }
    ...
    for robot in robots:
        ...
        # Check if robot has an active MQTT client connection
        robot_client = mqtt_manager.get_robot_client(robot_id)
        if robot_client:
            robot_status['connected'] = robot_client.connected
            robot_status['broker'] = f"{robot_client.broker_url}:{robot_client.port}"
            robot_status['message'] = 'Connected' if robot_client.connected else 'Disconnected'

        status_data['robots'][robot_id] = robot_status
